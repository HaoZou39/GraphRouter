...
def multihead_attention(queries,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        dense_Q = tf.keras.layers.Dense(units=num_units, activation=tf.nn.relu, name="Q_dense")
        dense_K = tf.keras.layers.Dense(units=num_units, activation=tf.nn.relu, name="K_dense")
        dense_V = tf.keras.layers.Dense(units=num_units, activation=tf.nn.relu, name="V_dense")
        # Apply Dense layers to the inputs
        Q = dense_Q(queries)  # (N, T_q, C)
        K = dense_K(keys)     # (N, T_k, C)
        V = dense_V(keys)     # (N, T_k, C)
