...
def create_network_graph(df, use_directed=False):
    """
    Create network graph from dataframe
    
    Args:
        df: GeoDataFrame with network data
        use_directed: If True, create directed graph with oneway support.
                     If False (default), create undirected graph with one edge per node pair.
    
    Returns:
        Tuple of (G_con, G_sel_con) - connected graphs
    """
    ...
    if use_directed:
        ...
        # Return undirected graphs (original momepy output)
        # This ensures only one edge per node pair
        return G_con, G_sel_con
